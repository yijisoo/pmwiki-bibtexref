...
class BibtexEntry {
  ...
  function getAuthors() {
    ...
    for ( $i = 0; $i < count( $aut ); $i++ ) {
      $ret = $ret . $this->getAbbreviatedName( $aut[$i] );
      if ( $i == count( $aut ) - 2 )
        $ret = $ret . ", & ";
  ...
  function getAbbreviatedName( $name ) {
    if (strpos($name, ",") > 0) {
      ...
      $space_position = strrpos($name, " ");
      $last = substr($name, $space_position);
      $first = substr($name, 0, $space_position);
    }
    ...
    for ( $i = 0; $i < count($first_parts); $i++) {
      if ($first_parts[$i] == " " || $first_parts[$i] == "-") {
        $first_abbrev = $first_abbrev . $first_parts[$i];
      }
      else if (strlen($first_parts[$i]) > 0) {
        $first_abbrev = $first_abbrev . $first_parts[$i][0] . ".";
      }
    }

    $ret = trim($last . ", " . $first_abbrev);
...
class Article extends BibtexEntry {
  ...
  function getSummary( $dourl = true ) {
    $ret = parent::getPreString( $dourl );
    $journal = parent::getFormat( "JOURNAL" );
    if ( $journal ) {
      $ret = $ret . " ''" . $journal . "''";
